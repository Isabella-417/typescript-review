...
export function funWithAnagrams(str: string[]) {
    const isAnagram = (initial, arrayToCompare) => {
        return arrayToCompare.filter(element => !(
            initial.length === element.length &&
            initial.split('').sort === element.split('').sort)
        )
    }
    ...
    for (let index = 0; index < str.length; index++) {
        ...
        newArray.indexOf(str[index]);
        newArray.splice(index, 1);

        const result = isAnagram(str[index], newArray);
        str = [str[index]].concat(result);
    }
